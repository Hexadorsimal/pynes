from nes.processors.cpu import InstructionFactory, ParameterReader, ResultWriter

from .arithmetic import Adc, And, Asl, Eor, Lsr, Ora, Rol, Ror, Sbc
from .branch import Bcc, Bcs, Beq, Bmi, Bne, Bpl, Bvc, Bvs
from .compare import Bit, Cmp, Cpx, Cpy
from .flags import Sec, Sed, Sei, Clc, Cld, Cli, Clv
from .increment import Inc, Inx, Iny, Dec, Dex, Dey
from .interrupt import Brk, Rti
from .jump import Jmp
from .load_store import Lda, Ldx, Ldy, Sta, Stx, Sty
from .nop import Nop
from .stack import Pha, Php, Pla, Plp
from .subroutine import Jsr, Rts
from .transfer import Tax, Tay, Tsx, Txa, Txs, Tya
from ..addressing_modes import AbsoluteParameterReader, AbsoluteXParameterReader, AbsoluteYParameterReader, \
    ImmediateParameterReader, IndexedIndirectParameterReader, IndirectParameterReader, IndirectIndexedParameterReader, \
    RelativeParameterReader, ZeroPageParameterReader, ZeroPageXParameterReader, ZeroPageYParameterReader


def get_parameter_reader(addressing_mode: str) -> ParameterReader:
    readers = {
        'Absolute': AbsoluteParameterReader,
        'AbsoluteX': AbsoluteXParameterReader,
        'AbsoluteY': AbsoluteYParameterReader,
        'Immediate': ImmediateParameterReader,
        'IndexedIndirect': IndexedIndirectParameterReader,
        'Indirect': IndirectParameterReader,
        'IndirectIndexed': IndirectIndexedParameterReader,
        'Relative': RelativeParameterReader,
        'ZeroPage': ZeroPageParameterReader,
        'ZeroPageX': ZeroPageXParameterReader,
        'ZeroPageY': ZeroPageYParameterReader,
    }

    return readers[addressing_mode]()


def get_result_writer(addressing_mode: str) -> ResultWriter:
    writers = {}

    return writers.get(addressing_mode)


factory = InstructionFactory(
    [
        Adc, And, Asl, Eor, Lsr, Ora, Rol, Ror, Sbc,
        Bcc, Bcs, Beq, Bmi, Bne, Bpl, Bvc, Bvs,
        Bit, Cmp, Cpx, Cpy, Nop,
        Inc, Inx, Iny, Dec, Dex, Dey,
        Brk, Jmp, Jsr, Rti, Rts,
        Lda, Ldx, Ldy, Sta, Stx, Sty,
        Sec, Sed, Sei, Clc, Cld, Cli, Clv,
        Pha, Php, Pla, Plp,
        Tax, Tay, Tsx, Txa, Txs, Tya,
    ],
    get_parameter_reader,
    get_result_writer,
)
