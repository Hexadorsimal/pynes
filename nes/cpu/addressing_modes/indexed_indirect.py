...
class IndexedIndirectAddressing(AddressingMode):
    def __init__(self):
        ...
        self.cycles.append(Cycle([RW(1), (ZeroPageAddress('BAL'), 'DL')]))
        self.cycles.append(Cycle([RW(1), (ZeroPageAddress('BAL + X'), 'ADL')]))
        self.cycles.append(Cycle([RW(1), (ZeroPageAddress('BAL + X + 1'), 'ADH')]))
